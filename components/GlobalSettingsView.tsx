      ...
      <div className="flex space-x-8 mb-10 border-b border-white/5">
        {[
          ...
          <button 
            ...
            className={`py-4 text-[12px] font-black uppercase tracking-[0.2em] relative transition-all ${activeTab === tab.id ? 'text-blue-400' : 'text-slate-600 hover:text-slate-400'}`}
          >
            {tab.label}
        ...
        {activeTab === 'env' && (
          <div className="space-y-8">
            <div className="flex justify-between items-center mb-4">
               ...
               <button className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-[11px] font-black uppercase tracking-widest text-blue-500 hover:bg-slate-800 transition-all">+ Add Variable</button>
            </div>
                ...
                {globalVars.map(v => (
                  <div key={v.id} className="p-6 flex items-center justify-between group hover:bg-white/[0.01] transition-all">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button className="text-[12px] font-bold text-slate-600 hover:text-white uppercase tracking-widest px-2">Edit</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-6 rounded-[2rem] bg-amber-500/5 border border-amber-500/20">
               <h4 className="text-[12px] font-black text-amber-500 uppercase tracking-widest mb-2 flex items-center">
                 ...
                 {[
                   ...
                   <div key={i} className="flex items-center justify-between p-4 bg-slate-950/50 border border-slate-800 rounded-2xl">
                     <div className="flex items-center space-x-4">
                        <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-[12px] font-bold text-slate-500">{u.user[0].toUpperCase()}</div>
                        <div>
                           <p className="text-xs font-bold text-slate-200">{u.user}</p>
                           <p className="text-[11px] text-slate-600 uppercase tracking-widest font-black">{u.scope}</p>
                        </div>
                     </div>
