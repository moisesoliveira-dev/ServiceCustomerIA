    ...
    <div className="relative" ref={switcherRef}>
      ...
      >
        <div className="flex items-center space-x-3 overflow-hidden">
          <div className={`w-9 h-9 rounded-xl ${activeCompany.color} flex items-center justify-center text-xs font-black text-white shrink-0`}>
            {activeCompany.name[0]}
          </div>
          <div className="overflow-hidden">
            <p className="text-sm font-bold text-white truncate">{activeCompany.name}</p>
            <p className="text-[11px] text-slate-500 uppercase font-black truncate">ID: {activeCompany.id.substring(0, 8)}</p>
          </div>
        </div>
            ...
            {companies.map(company => (
              <button 
                ...
                className={`w-full flex items-center space-x-3 p-3 rounded-lg text-left transition-colors ${
                  activeCompany.id === company.id ? 'bg-blue-600/10 text-blue-300' : 'hover:bg-slate-800/50'
                }`}
              >
                <div className={`w-8 h-8 rounded-lg ${company.color} flex items-center justify-center text-xs font-black text-white shrink-0`}>
                  {company.name[0]}
                </div>
    ...
    <>
      <aside className="w-64 border-r border-white/5 flex flex-col bg-[#050811] shrink-0">
        ...
        <div className="flex-1 px-4 overflow-y-auto py-4 custom-scrollbar">
          <div className="mb-3 px-4 text-[11px] font-black text-slate-600 uppercase tracking-widest">Plataforma (ADM)</div>
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center justify-between p-2 rounded-xl bg-white/[0.02]">
            ...
            <button onClick={() => setShowLogoutConfirm(true)} className="p-2 text-slate-600 hover:text-rose-500 transition-colors rounded-lg hover:bg-rose-500/10" title="Logout">
              <Icons.Logout />
            </button>
          </div>
        </div>
