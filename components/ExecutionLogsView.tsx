  ...
  return (
    <div className="relative flex gap-8">
      ...
      <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center z-10 shrink-0 ${isFailed ? 'bg-rose-500 border-rose-950 shadow-[0_0_10px_#f43f5e]' : 'bg-slate-900 border-slate-950'}`}>
        {isFailed ? <span className="text-white text-[8px] font-black">!</span> : <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div>}
      </div>
      <div className="flex-1 pb-10">
        ...
        <div className={`p-4 rounded-2xl bg-slate-950/50 border border-slate-800/50 transition-all ${isFailed ? 'border-rose-500/20' : ''}`}>
           <div className="flex justify-between items-center mb-4">
             ...
             <button onClick={() => setShowPayload(!showPayload)} className="text-[10px] font-bold text-blue-500 uppercase tracking-widest hover:underline">{showPayload ? 'Hide Trace' : 'Show Trace'}</button>
           </div>
             ...
             {showPayload && (
               <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-4 overflow-hidden">
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <p className="text-[9px] font-black text-slate-700 uppercase tracking-widest mb-1">Payload In</p>
                       <pre className="p-3 bg-slate-900 rounded-lg text-[10px] font-mono text-emerald-500 truncate">{JSON.stringify(step.payloadIn, null, 2)}</pre>
                    </div>
                    <div>
                       <p className="text-[9px] font-black text-slate-700 uppercase tracking-widest mb-1">Payload Out (Masked)</p>
                       <pre className="p-3 bg-slate-900 rounded-lg text-[10px] font-mono text-blue-400 truncate">
          ...
          {MOCK_EXECUTIONS.map(log => (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-mono font-bold text-slate-500">{log.id}</span>
                <Badge color={log.status === 'SUCCESS' ? 'emerald' : 'rose'}>{log.status}</Badge>
              </div>
              <p className="text-[11px] font-black text-slate-300 uppercase tracking-widest truncate">{log.sessionId}</p>
              <div className="flex justify-between items-center mt-3">
                <span className="text-[10px] text-slate-600 font-bold italic">{log.duration}</span>
                <span className="text-[9px] text-slate-700 font-mono">{log.timestamp.split(' ')[1]}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 p-10 overflow-y-auto custom-scrollbar bg-[#0b1120]/40">
        {selectedLog ? (
          <div className="max-w-4xl mx-auto">
            <header className="mb-12 flex justify-between items-end border-b border-white/5 pb-8">
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <span className="px-3 py-1 bg-slate-800 rounded-lg text-[10px] font-mono text-slate-400 border border-white/5">{selectedLog.id}</span>
                </div>
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Observabilidade Granular de Pipeline AI</p>
              </div>
              <div className="flex gap-4">
                 <div className="text-right">
                   <p className="text-[9px] font-black text-slate-700 uppercase tracking-widest">Tempo Total</p>
                   <p className="text-lg font-black text-blue-500">{selectedLog.duration}</p>
                 </div>
                 <div className="text-right">
                   <p className="text-[9px] font-black text-slate-700 uppercase tracking-widest">Sessão</p>
            ...
            <div className="mt-12 p-6 rounded-3xl bg-indigo-500/5 border border-indigo-500/20">
               <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Análise de Performance</h5>
          ...
          <div className="h-full flex items-center justify-center opacity-20">
             <p className="text-[10px] font-black uppercase tracking-[0.3em]">Selecione uma execução para ver o trace</p>
          </div>
        )}
      </div>
    </div>
