    ...
    <div className="flex-1 p-10 bg-[#02040a] overflow-y-auto custom-scrollbar">
      <SectionHeader 
        ...
        action={
          <button 
            ...
            className="px-6 py-2.5 bg-blue-600 text-white text-[12px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20"
          >
            + New Destination
          </button>
        }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-4">
          <h3 className="text-[12px] font-black text-slate-600 uppercase tracking-widest px-2">Configured Egress</h3>
          {routes.map(route => (
            <button 
              key={route.id} 
                ...
                <div className="flex space-x-6 border-b border-white/5 mb-6">
                  {['config', 'history'].map(tab => (
                    <button 
                      ...
                      className={`pb-4 text-[12px] font-black uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-blue-400' : 'text-slate-600'}`}
                    >
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                      ...
                      <Card className="border-slate-800">
                        <div className="space-y-4">
                          <div className="flex gap-4">
                            <div className="w-28">
                              <label className="text-[11px] font-black text-slate-600 uppercase mb-1 block">Method</label>
                              <select 
                            ...
                            <div className="flex-1">
                              <label className="text-[11px] font-black text-slate-600 uppercase mb-1 block">Target URL</label>
                              <input 
                          ...
                          <div className="h-64 flex flex-col">
                            <label className="text-[11px] font-black text-slate-600 uppercase mb-2 block">Body Template (JSON)</label>
                            <CodeEditor 
                              label="Dynamic Body" 
                          ...
                          <div className="flex justify-between items-center pt-4">
                             <button onClick={runSimulation} disabled={isSimulating} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[12px] font-black uppercase tracking-widest rounded-xl transition-all">
                               {isSimulating ? 'Sending Request...' : 'Simular Request'}
                             </button>
                             <div className="flex gap-2">
                               <button onClick={() => setEditingRoute(null)} className="px-4 text-[12px] font-bold text-slate-500">Cancel</button>
                               <button onClick={handleSave} className="px-6 py-2 bg-blue-600 text-white text-[12px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-blue-600/20">Save Route</button>
                             </div>
                          </div>
                        </div>
                      </Card>
                        ...
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                           <Card className="border-emerald-500/20 bg-emerald-500/[0.02]">
                              <div className="flex justify-between items-center mb-4">
                                <span className="text-[12px] font-black text-emerald-500 uppercase tracking-widest">Simulation Success</span>
                    ...
                    <div className="space-y-6">
                      <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-6">
                        <h4 className="text-[12px] font-black text-slate-500 uppercase tracking-widest mb-6">Variable Assistant</h4>
                        <div className="space-y-3">
                          {AVAILABLE_VARIABLES.map(v => (
                            ...
                            >
                              <code className="text-[12px] text-blue-400 font-bold block mb-1 group-hover:text-blue-300">{v.key}</code>
                              <span className="text-[11px] text-slate-600 uppercase font-black">{v.description}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="p-6 bg-amber-500/5 border border-amber-500/10 rounded-[2rem]">
                         <p className="text-[11px] text-amber-500/60 font-black uppercase leading-relaxed tracking-wider">
                        ...
                        <div key={idx} className="p-6 flex items-center justify-between hover:bg-white/[0.01] transition-colors">
                           <div className="flex items-center space-x-6">
                              ...
                              <div>
                                ...
                                <p className="text-[11px] text-slate-600 font-mono mt-1">{exec.timestamp}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <p className="text-[12px] font-black text-slate-400 uppercase tracking-widest">Latency</p>
